...
option problem is a version of binary classification where outputs
...
where both items/documents are judged to be equally good ($y_i=0$).
This has been called the ``ranking with ties'' problem
\cite{rank-with-ties}, proposed a boosting algorithm, and observed
that modeling ties is more effective when there are more output
values. A Bayesian model which can be applied to these data is
TrueSkill \cite{trueskill}, which a generalization of the Elo chess
rating system.
...
$\hat \mu,\hat w$, we recover the same constraints as the max margin
comparison LP (\ref{eq:max-margin-lp}). Thus $\hat \mu, \hat w$ are
feasible for (\ref{eq:max-margin-lp}).
\end{proof}
...
$x,x'$. However, when the input variables are scaled in a
pre-processing step, we have observed that the solutions to the LP and
QP are equivalent (right panel of Figure~\ref{fig:hard-margin}).
